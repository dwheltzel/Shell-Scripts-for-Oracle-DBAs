...
usage() {
      echo "Usage: $0 [-d database name] [-m email address]"
...
COL realsize_gb FOR 99,999.9
COL used_gb FOR 99,999.9
COL free_gb FOR 99,999.9
COL pct_free FOR 999.9
set head on
SELECT a.tablespace_name,round(a.realsize_bytes/1024/1024/1024,1) realsize_gb,round(b.tot_bytes/1024/1024/1024,1) used_gb,
  ROUND((a.realsize_bytes-b.tot_bytes)/1024/1024/1024,1) free_gb,ROUND((a.realsize_bytes-b.tot_bytes)/a.realsize_bytes*100,1) pct_free FROM
(SELECT tablespace_name,SUM(realsize) realsize_bytes FROM
(SELECT tablespace_name, bytes, autoextensible, maxbytes, CASE WHEN autoextensible = 'YES' THEN maxbytes ELSE bytes END realsize FROM dba_data_files) GROUP BY tablespace_name) a
JOIN
(SELECT tablespace_name,SUM(bytes) tot_bytes FROM dba_extents GROUP BY tablespace_name) b
ON (a.tablespace_name = b.tablespace_name) ORDER BY 5;
set head off
prompt
...
SELECT 'exec DBMS_STATS.GATHER_TABLE_STATS('''||t.owner||''','''||t.table_name||''');' FROM dba_tables t
 ...
 WHERE t.temporary = 'N' AND t.secondary = 'N' AND t.segment_created = 'YES'
 --AND t.owner NOT LIKE '%SYS%' AND t.owner NOT IN ('XDB')
 AND t.owner IN (SELECT username FROM dba_users WHERE profile = 'DATACCT')
...
SELECT 'alter user ' || username || ' temporary tablespace TEMP;' FROM dba_users u WHERE temporary_tablespace <> 'TEMP';
